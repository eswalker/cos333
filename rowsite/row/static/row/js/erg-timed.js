...
$('._submit').click(function() {
	...
	in_seconds = parseFloat($("#in_second").val());
	in_minutes = parseInt($("#in_minute").val());
	timeString = in_minutes + ":" + in_seconds;
	var time = 0;
	if (!(in_seconds || in_minutes)) {
		valid =false;
		$("#distance").parent().removeClass("_erg_valid");
		$("#distance").parent().addClass("_erg_error");
	} else {
		time = in_minutes * 60 + in_seconds;
	...
	if (valid) {
		...
		var posted_data = { results : s , name : timeString};
